...
A Raspberry Pi 4B is connected to three storage devices: (1) the original 32GB sdcard that now only serves as a boot partition, (2) a 240GB M.2 Western Digital SSD connected through an M.2 to USB3 adapter card - [**NexStar SX M.2 SSD to USB 3.0 Enclosure**](https://www.vantecusa.com/products_detail.php?p_id=214), and (3) a 1TB 2.5" Toshiba hdd connected through a SATA to USBC converter - [**ORICO 2.5 inch Transparent Type-C Hard Drive Enclosure**](http://my.orico.cc/goods.php?id=6352). The M.2 to USB3 converter use an ASMedia chipset which can mean it will do uas without errors - or not. Refer to [**this bugzilla.redhat snippet from Hans de Goede about 5 years ago**](ASMedia-usb3-to-sata-bridges-quirks.txt). It is the same [**Hans de Goede**](https://github.com/TobiasVanDyk/gkrellm-uguru-for-GKrellM2) that I tangentially had a positive but delayed, indirect encounter with in 2005 regarding Abit uguru motherboard drivers for lmsensors. SATA to USB3 chipsets that has broken UAS support (which is essential for SSD TRIM operations), can be handled better by keeping them in a USB3 hub but only using the two USB2 data lines, compared to plugging then into a Raspberry Pi USB2 hub. 
